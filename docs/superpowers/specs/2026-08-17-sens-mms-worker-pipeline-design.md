...
| 명시적 실패 재시도 간격 | 10초 |
| 전역 HTTP 429 백오프 | 첫 429는 10초, 두 번째 이후는 매번 최대 20초 |
...
- `sens_mms/coordination.py`: worker가 공유하는 체크포인트, 이벤트, 전역 중단, 전역 429 백오프 조정자
- `sens_mms/pipeline.py`: 한 수신번호의 예약, MMS POST, 상관관계 확인, 폴링과 재시도 상태 전이
- `sens_mms/workflow.py`: 승인 검증, pending-first 단계, 조건부 이미지 업로드, 최대 5 worker 실행, 완료 집계
- `sens_mms/preflight.py`: 8열 현재 상태를 승인 작업으로 고정하고 승인 토큰에 대상, 이미지 해시, 본문과 실행 설정을 결합
...
1. 프로젝트 live-execution lock을 획득하고 승인 토큰을 검증한다.
2. 기존 8열 결과를 읽고 모든 `PENDING_CONFIRMATION` 작업을 새 발송보다 먼저 최대 5개씩 병렬 조회한다.
3. pending 조회 future를 모두 합류시키고 최신 결과를 다시 검증한다.
4. 승인 범위에 새 POST가 하나라도 있을 때만 사전 검증에서 보존한 불변 이미지 바이트를 각각 한 번 업로드한다.
5. 신규 및 승인된 실패 재발송 행을 POST 전 예약 상태로 원자적으로 저장한다.
6. 최대 5개의 worker가 각 수신번호의 통합 MMS를 성공, 최종 실패 또는 pending까지 전담한다.
7. 모든 future를 합류시킨 뒤 불변 스냅샷에서 완료 집계를 계산한다.

pending-only 실행에서는 이미지를 업로드하지 않는다. 이미지 업로드가 실패하면 새 MMS POST를 시작하지 않는다. 최종 이미지 승인 토큰 검증 뒤에는 경로를 다시 열지 않으며 메타데이터, 해시, 업로드 Base64는 모두 같은 불변 바이트에서 파생한다.
...
명시적인 비접수 POST 실패 또는 상관관계가 확인된 `COMPLETED + fail`만 재POST를 허용한다. 재시도 전에 10초를 기다리고 최초 시도 포함 최대 3회까지만 POST한다. 세 번째 명시적 실패 후 `FAILED`로 확정한다.
...
- 이미 접수된 요청의 확인은 가능한 상태를 보존하되 중단 이후 새 POST는 금지
- 한 번호의 정상적인 명시적 실패는 다른 번호를 중단하지 않음

HTTP 429는 실행 전체에 적용한다.

- 첫 429 후 모든 새 API 호출을 10초 보류
- 같은 실행의 두 번째 이후 429는 매번 최대 20초 보류
...
preflight는 전체 대상 수, pending 조회 수, 신규/실패 재발송 수, 마스킹된 번호 표본, 등록 확인할 발신번호, 정확한 본문, 이미지 파일명·크기·해상도·SHA-256과 순서, `MMS`, `contentType`, worker 5개와 1/120/10/3 및 429 설정을 표시한다.
...
실제 SENS API나 실제 수신번호를 사용하지 않고 모의 transport와 제어 가능한 clock으로 검증한다.
...
- pending-only 실행의 이미지 업로드 생략과 업로드 실패 시 모든 새 POST 차단
- worker 최대 동시성 5, 수신번호별 전담 처리와 결과/로그 직렬화
- 전역 429 첫 10초·이후 최대 20초와 GET/POST 분류
- 체크포인트·이벤트 실패 후 모든 later new POST 차단
...
기존 LMS→MMS 설계와 구현 계획은 삭제하지 않고 새 설계로 대체되었음을 표시한다. 실험 worktree와 branch 이름은 의사결정 이력과 기존 격리를 보존하기 위해 유지한다. `AGENTS.md`, 운영 정책, README와 새 구현 계획은 통합 MMS, 8열 결과 및 고정 worker/timing 설정에 맞춘다.

...
6. 각 수신번호는 독립된 MMS POST 하나를 사용한다. `messages`에는 수신번호 한 건만 있고, 승인된 `message.txt` 전체와 `contentType=COMM`을 그대로 사용하며 `subject`를 넣지 않는다. 워커는 최대 2개이며, POST와 조회를 분리해 동시에 미확인 요청을 최대 8건까지 유지한다. 목록 조회가 이미 `COMPLETED`이면 결과 GET을 생략한다.

첨부 업로드가 실패하면 업로드를 재시도하지 않고 메시지 POST도 시작하지 않는다. 업로드 429도 동일하다. 모든 API 호출은 한 실행에서 공유되는 전역 속도 제한과 429 게이트를 통과한다. 호출 사이 최소 0.5초를 두고, 첫 429 뒤 10초, 이후 각 429 뒤 20초 동안 새 API 호출을 막는다. 명시적 전송 실패에 대한 10초 재시도 대기는 이 전역 게이트와 중복해서 더하지 않는다.

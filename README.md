...
6. 각 수신번호는 독립된 MMS POST 하나를 사용한다. `messages`에는 수신번호 한 건만 있고, 승인된 UTF-8 본문과 `contentType=COMM`을 그대로 사용하며 `subject`를 넣지 않는다. 워커는 최대 2개이며, POST와 조회를 분리해 동시에 미확인 요청을 최대 8건까지 유지한다. 목록 조회가 이미 `COMPLETED`이면 결과 GET을 생략한다.
